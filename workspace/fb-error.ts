__d("fb-error", ["performanceNowSinceAppStart", "removeFromArray"], (function (a, b, c, d, e, f) {
    ...
    function FBLogger(logInfo) {
        let messageFormat = logInfo.messageFormat || logInfo.message;
        let messageParams = logInfo.messageParams || [];
        return formatString(messageFormat, messageParams);
    }

    function formatString(message, params) {
        let index = 0;
        message = message.replace(/%s/g, () => {
            return index < params.length ? params[index++] : "NOPARAM";
        });
        if (index < params.length) {
            message += ` PARAMS ${JSON.stringify(params.slice(index))}`;
        }
        return message;
    }
    function f(a) {
