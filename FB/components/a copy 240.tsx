...
type CookieType = {
  ...
  setWithoutCheckingUserConsent_DANGEROUS: (name: string, value: string) => void;
  setWithoutChecks: (
    name: string,
    value: string,
    expires?: number,
...
function hasUserConsent(feature: number): boolean {
  if (!(CookieConsent_ || (CookieConsent_ = new CookieConsent())).hasConsent(feature)) {
...
function noCookiesAllowed(): boolean {
  return !(InitialCookieConsent_ || (InitialCookieConsent_ = new InitialCookieConsent())).noCookies;
}
...
const Cookie: CookieType = {
  ...
  setWithoutChecks: (
    ...
    if (!noCookiesAllowed() && hasUserConsent(1)) {
      CookieCore.setWithoutChecks(name, value, expires, path, domain, secure, sameSite);
    }
  },
  setWithoutChecksIfFirstPartyContext: CookieCore.setWithoutChecksIfFirstPartyContext,
};
