...
interface BaseThemeProviderProps {
  children: (theme: React.CSSProperties, variables: React.CSSProperties) => React.ReactNode;
  config?: {
...
const BaseThemeProvider: React.FC<BaseThemeProviderProps> = ({
  ...
  const parentConfig = useContext(BaseThemeConfigContext);
  ...
  const theme = useMemo(() => {
    ...
    if (config !== null && config.type === 'CLASSNAMES') {
      ...
      className = mode === 'dark' ? parentConfig.darkClassName : parentConfig.lightClassName;
    }
    ...
    return null;
  }, [config, mode, parentConfig.darkClassName, parentConfig.lightClassName]);
  ...
  const newConfig = useMemo(() => {
    if (config !== null && config?.type === 'VARIABLES') {
      return {
        ...parentConfig,
        darkVariables: {
          ...parentConfig.darkVariables,
          ...config.dark  
        },
        lightVariables: {
          ...parentConfig.lightVariables,  
          ...config.light
        }
      };
  ...
  const variables = useMemo(() => {
    const vars = mode === 'dark' ? newConfig.darkVariables : newConfig.lightVariables;
