...
type DomainFilter = (domain: string) => string;
type QueryParamsFilter = (domain: string, params: Map<string, string>) => Map<string, string>;

interface ConstUriComponents {
  domain: string;
  fragment: string;
  ...
  serializer: PHPQuerySerializer | PHPQuerySerializerNoEncoding;
  subdomain: string;
}

interface ConstUriValidation {
  components: ConstUriComponents;
  error: string;
  valid: boolean; 
}
...
class ConstUri {
  ...
  static domainFilters: DomainFilter[] = [];
  static queryParamsFilters: QueryParamsFilter[] = [];

  queryParams: Map<string, string>;

  constructor(components: ConstUriComponents) {
    this.queryParams = new Map();
    this.domain = components.domain;
    this.fragment = components.fragment;
    this.fragmentSeparator = Boolean(components.fragmentSeparator);
    this.isGenericProtocol = Boolean(components.isGeneric);
  ...
  static getUri(uriString: string): ConstUri | null {
    // ...implementation
  }

  static getUribyObject(components: ConstUriComponents, hasQuery: boolean): ConstUri | null {
    // ...implementation 
  }

  static isValidUri(uriString: string): boolean {
    // ...implementation
  }

  static registerDomainFilter(filter: DomainFilter) {
    ConstUri.domainFilters.push(filter);
  }

  static registerQueryParamsFilter(filter: QueryParamsFilter) {
    ConstUri.queryParamsFilters.push(filter);
  }
...
export {
  ...
  isSubdomainOfDomain,
  isConstUri,
  registerDomainFilter, 
  registerQueryParamsFilter,
  getUri,
  isValidUri
};